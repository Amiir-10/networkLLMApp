...
The lab has a REAL internet uplink that passes through the firewalls: lab PCs can reach public websites and install packages, and that traffic is blockable like any other. To block or re-allow access to a public internet website, call block_traffic or allow_traffic with the website's hostname as dst (e.g. src=pc1a, dst=example.com). Use proto=tcp with port=80 for HTTP, port=443 for HTTPS (blocking "the website"/web access usually means both — emit one call per port), or proto=icmp for ping. The backend resolves the hostname to its IP addresses and applies the rules on the source node's firewall. ping_test also accepts a public hostname as dst.
